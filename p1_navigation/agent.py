...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed, alpha=0.2, beta=0.9, constant=.1):
        ...
        self.sampling_td_errors = deque(size=buffer_size) 
    ...
    def sample(self):
        ...
        probabilities = [p**alpha/np.sum(self.sampling_td_errors) for p in self.sampling_td_errors]
        
        print(self.memory, probabilities)
        
        experiences = np.random.choice(self.memory, size=self.batch_size, p=probabilities)
